...
export const ClientManagement: React.FC = () => {
  ...
  const verifyGoogleEmail = async (emailToVerify: string) => {
    ...
    if (isAuthorizedGmail(normalizedEmail)) {
      ...
      setAuthError(`Access Denied — The Google account (${normalizedEmail || 'unknown'}) is not authorized to access Client Management.`);
    }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const updated = await deleteClientRecord(clientToDelete.id);
      setClients(updated);
      showToast(`Deleted client record for "${clientToDelete.name}"`);
    } catch (err) {
      ...
      <div className="py-8 px-3 sm:px-6 max-w-xl mx-auto text-center">
        ...
        >
          ...
          <button
            ...
            className="w-full py-3.5 px-6 rounded-2xl bg-[#8C1D18] hover:bg-[#6e1612] text-white font-bold text-sm shadow-md transition-all flex items-center justify-center gap-3 cursor-pointer disabled:opacity-50 mb-4"
          >
            {isSigningIn ? (
              <>
          ...
          <div className="relative my-6 flex items-center justify-center">
            ...
            <span className="relative bg-[#FAF8F5] px-3 text-xs text-stone-400 font-medium uppercase tracking-wider">
              or verify authorized gmail
            </span>
          </div>
          ...
          >
            <div>
              ...
              <input
                ...
                placeholder="Enter authorized google email address..."
                className="w-full px-4 py-2.5 rounded-xl border border-stone-300 text-xs text-stone-900 bg-white placeholder:text-stone-400 focus:outline-hidden focus:border-[#8C1D18] focus:ring-2 focus:ring-[#8C1D18]/20 transition-all font-mono"
              />
            </div>

            <button
              type="submit"
              disabled={!customEmailInput.trim() || isSigningIn}
              className="w-full py-2.5 px-4 rounded-xl bg-stone-800 hover:bg-stone-900 text-white font-semibold text-xs shadow-xs transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
            >
              {isSigningIn ? (
                <>
